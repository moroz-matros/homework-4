...
class SearchPage(Page):
    ...
    def search(self, text):
        str = self.wait_until_and_get_elem_by_css(self.SEARCH_INPUT)
        button = self.wait_until_and_get_elem_by_css(self.SEARCH_BUTTON)
        str.send_keys(text)
        button.click()
    ...
    def get_active_tab_title(self):
        tab = self.wait_until_and_get_elem_by_css(self.ACTIVE_TAB)
        return tab.text

    def redirect_next_page(self):
        button = self.wait_until_and_get_elem_by_css(self.PAGE_NEXT_BUTTON)
        button.click()

    def get_page_num(self):
        num = self.wait_until_and_get_elem_by_css(self.PAGE_NUM)
        return num.text

    def redirect_to_users(self):
        tab = self.wait_until_and_get_elem_by_css(self.USERS_TAB)
        tab.click()

    def redirect_to_first_user(self):
        link = self.wait_until_and_get_elem_by_css(self.USER_NAME)
        link.click()
    ...
    def get_first_user(self):
        user = self.wait_until_and_get_elem_by_css(self.USER)
        return user

    def redirect_to_main(self):
        button = self.wait_until_and_get_elem_by_css(self.BACK_BUTTON)
        button.click()
